...
namespace ICSI_eCSIN.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public bool CheckUdn(tblUser objuser) //This method check the ecSIN existence
        {
            bool flag = false;
        ...
        public int updatetblUserById(tblUser objtblUser)
        {
            ...
            string body = "Dear " + restblUser.FirstName + ",<br/><br/> Your Login Credentials are  as follows :-  LoginID :- " + restblUser.UserName + " and Password:- " + restblUser.Password + ".";
        ...
        public tblUser GetUserByID(int UserId)
        {
            ...
            return DBcontext.tblUsers.Where(X => X.UserId == UserId).FirstOrDefault();
        }
            ...
            sbString.Append("<!DOCTYPE html>"
                ...
                + "<body>"
                    ...
                    + "<table style=\"font-family:arial,sans-serif;border-collapse:collapse;width:100%;font-size:13px;\">"
                        ...
                        + "</tr>"
                        + "<tr style=\"background-color:#dddddd\">"
                            + "<th style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #2D4383\">Total annual emoluments (cost to company)</th>"
                            + "<td style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #666\">" + objeCSINGeneration.Amount + "</td>"
                        + "</tr>"
                         + "<tr>"
                            + "<th style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #2D4383\">Remarks</th>"
                            + "<td style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #666\">" + objeCSINGeneration.Remarks + "</td>"
                        + "</tr>"
                    + "</table>"
                + "</body>"
                + "</html>");
        ...
        public string SubeCSINGenerationEmailBody(SubeCSINGeneration objeCSINGeneration)
        {
            string CINTitle = string.Empty;
            string CINValue = string.Empty;
            if (objeCSINGeneration.Number == "1")
            ...
            sbString.Append("<!DOCTYPE html>"
                ...
                + "<body>"
                    ...
                    + "<table style=\"font-family:arial,sans-serif;border-collapse:collapse;width:100%;font-size:13px;\">"
                        ...
                        + "<tr style=\"background-color:#dddddd\">"
                           + "<th style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #2D4383\"> Name and eCSIN of holding Company</th>"
                            + "<td style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #666\">" + objeCSINGeneration.EmployerName + "</td>"
                        + "</tr>"

                          + "<tr style=\"background-color:#dddddd\">"
                           + "<th style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #2D4383\">Name of Subsidiary Company </th>"
                            + "<td style=\"border: 1px solid #dddddd;text-align: left;padding: 8px;color: #666\">" + objeCSINGeneration.SubEmployerName + "</td>"
                        + "</tr>"
            ...
            if (obj.FromDate != null)
            {
                ...
                var result = (from eCSIN in DBcontext.tbleCSINGenerations
                              ...
                              select new eCSINDetails
                              {
                                  ...
                                  MemberNumber = user.UserName,
                                  WebsiteOfEmployer= eCSIN.WebsiteOfEmployer,
                                  EmployerEmailId = eCSIN.EmployerEmailId,
                                  PANNumber = eCSIN.PANNumber,         //Amount = Convert.ToDecimal(eCSIN.Amount),
            ...
            else
            {
                ...
                var result = (from eCSIN in DBcontext.tbleCSINGenerations
                              ...
                              select new eCSINDetails
                              {
                                  ...
                                  WebsiteOfEmployer = eCSIN.WebsiteOfEmployer,
                                  EmployerEmailId = eCSIN.EmployerEmailId,
                                  PANNumber = eCSIN.PANNumber, 
                                  RevokeDate=eCSIN.RevokeDate,
                                  EmployeeDesignation=eCSIN.EmployeeDesignation,//Amount = Convert.ToDecimal(eCSIN.Amount),
        ...
        public UpdateDetails GetEmployeeUpdateDetails(string eCSINNumber)
        {
            var reseCSINDetails = (from objuser in DBcontext.tblUsers.AsEnumerable()
                                   ...
                                   select new UpdateDetails
                                   {
                                       ...
                                       Remarks1 = objeCSINDetails.Remarks,
                                       Remarks2 = objeCSINDetails.Remarks2,
                                       Remarks3= objeCSINDetails.Remarks3,
                                       Remarks4=objeCSINDetails.Remarks4,
                                       Remarks5=objeCSINDetails.Remarks5
                                   }).SingleOrDefault();
            return reseCSINDetails;
        }
            ...
            try
            {
                ...
                if (reseCSIN != null)
                {
                    ...
                    if (reseCSIN.eCSINGeneratedNo != null)
                    {
                        ...
                        reseCSIN.Remarks2 = objUpdateCSIN.Remarks2;
                        reseCSIN.Remarks3 = objUpdateCSIN.Remarks3;
                        reseCSIN.Remarks4 = objUpdateCSIN.Remarks4;
                        reseCSIN.Remarks5 = objUpdateCSIN.Remarks5;
                        status = DBcontext.SaveChanges();
                    }
                }
            }
        ...
        public bool checkExistRevokedeCSINforMSub(string RevokedeCSINNumber)   /*Check for multiple eCSIN generation even after Cessation of 2nd*/
        {
            bool status = false;

            var reseCSIN = DBcontext.tbleCSINGenerations.Where(x => x.eCSINGeneratedNo == RevokedeCSINNumber && x.Subsidiarye_Status == true).FirstOrDefault();
            if (reseCSIN != null)
                status = true;
        ...
        public string GetExistEmployerName(int UserId)
        {
            var reseCSIN = DBcontext.tbleCSINGenerations.Where(x => x.UserId == UserId && x.Status == true && x.Subsidiarye_Status == null).FirstOrDefault();
            if (reseCSIN != null)
                return reseCSIN.EmployerName;

            return null;
        }

        public bool checkExistSameUser(int objuserId , string objeCSINNumber)   /*Check for exist eCSIN for same user */
        {
            bool status = false;

            var reseCSIN = DBcontext.tbleCSINGenerations.Where(x => x.UserId == objuserId && x.eCSINGeneratedNo== objeCSINNumber).FirstOrDefault();
            if (reseCSIN != null)
                status = true;
        ...
        //for Admin Panel
        public bool CheckMembershipUpdate(int UserId)
        {
            bool flag = false;
            var eCSINId = (from cust in DBcontext.tblUsers
                           join ord in DBcontext.tbleCSINGenerations
            ...
            //tbleCSINGeneration tblObj = DBcontext.tbleCSINGenerations.Where(x => x.UserId == UserId).FirstOrDefault();
            //if (tblObj != null)
            //{
            //    return true;
            //}
        ...
        public List<eCSINDetails> GeteAllMembereCSINList(int UserId)
        {
                var result = (from eCSIN in DBcontext.tbleCSINGenerations
                              join user in DBcontext.tblUsers
                              on eCSIN.UserId equals user.UserId
        ...
        public int EmployeeeCSINUpdateByAdmin(eCSINDetails objeCSINUpdate)
        {
            int status = 0;
            try
            {

                var reseCSIN = DBcontext.tbleCSINGenerations.Where(x => x.eCSINGeneratedNo == objeCSINUpdate.eCSINGeneratedNo).FirstOrDefault();
                if (reseCSIN != null)
                {
                    
                    if (reseCSIN.eCSINGeneratedNo != null)
                    {                        
                        reseCSIN.EmployerName = objeCSINUpdate.EmployerName;       //reseCSIN.eCSINGenerationId = reseCSIN.eCSINGenerationId;                      
                        reseCSIN.EmployerCINNo = objeCSINUpdate.EmployerCINNo;
                        if (reseCSIN.EmployerCINNo ==null)
                        {
                            reseCSIN.EmployerCINNo = string.Empty;
                        } 
                        reseCSIN.PANNumber = objeCSINUpdate.PANNumber;
                        reseCSIN.WebsiteOfEmployer = objeCSINUpdate.WebsiteOfEmployer; 
                        status = DBcontext.SaveChanges();
                    }
                }
            }
        ...
        public int checkSubMemberExist(tblSubsidiaryUser UserMember)  // Subsidiary member registred throguh Admin
        {
            int status = 0;
            try
            {
                var MemTblUser = DBcontext.tblSubsidiaryUsers.Where(x => x.Membership_Number == UserMember.Membership_Number).FirstOrDefault();
                if (MemTblUser.Membership_Number != null)
                {
                    status = 1;
                }
            }
        ...
        public int InsertTblSubUser(tblSubsidiaryUser User)    // Entry for Subsidiary membership
        {
            int status = 0;
            try
            {
                var resTblUser = DBcontext.tblSubsidiaryUsers.Where(x => x.Membership_Number == User.Membership_Number).FirstOrDefault();

                if (resTblUser == null)
                {
                    DBcontext.tblSubsidiaryUsers.Add(User);
                    status = DBcontext.SaveChanges();
                }
            }
            catch (Exception ex)
