...
namespace ICSI_WebApp.Controllers
{
    public class ROController : Controller
    {
        ...
        public ActionResult RO(FormCollection frm)
        {
            ...
            if (Session["LOGIN_ID"] != null && Session["USER_MENU"] != null)
            {
                ...
                else return RedirectToAction("Home", "RO");
            }
            else
        ...
        public ActionResult logoutPortal()
        {
            Session.Clear();
            Session.Abandon();
            return RedirectToAction("DelegatePortal", "Home");
        }

        [HttpGet]
        public ActionResult Home()
        {
            if (Session["LOGIN_ID"] != null && Session["USER_MENU"] != null && ((List<object>)Session["USER_MENU"]).Count > 0 && Convert.ToString(Session["LOGIN_ID"]) != "roanonymous")
            {
        ...
        public ActionResult Delegate(FormCollection frm)
        {
            if (Session["LOGIN_ID"] == null)
            {
                login();
            ...
            if (Session["LOGIN_ID"] != null && Session["USER_MENU"] != null)
            {
                ...
                else return RedirectToAction("Delegate");
            }
            else
                return RedirectToAction("logoutPortal");
        }
        ...
        public JsonResult AjaxData()
        {
            if (Request.Form["Comp_ID"] != null)
            {
                ...
                schema = Util.UtilService.getSchemaNameById(8);
                    ...
                    else*/
                    {
                        ...
                        if (userid != null && !userid.Trim().Equals("") && ((menuid != null && !menuid.Trim().Equals("")) || (screenId != null && !screenId.Trim().Equals(""))))
                        {
                            ...
                            if (screen != null)
                            {
                                if (screen.ID == 486)
                                {
                                    string qrText = frm["UNIQUE_REG_ID"].ToString();
                                    string qrimg= frm["ui"].ToString();
                                    QRCodeGenerator qrGenerator = new QRCodeGenerator();
                                    QRCodeData qrCodeData = qrGenerator.CreateQrCode(qrText, QRCodeGenerator.ECCLevel.Q);
                                    QRCode qrCode = new QRCode(qrCodeData);
                                    Bitmap qrCodeImage = qrCode.GetGraphic(20);
                                    string folderPath = Server.MapPath("~/QRimage/");
                                    string fileName = ""+ qrimg + ".jpg";
                                    string imagePath = folderPath + fileName;
                                    qrCodeImage.Save(imagePath, System.Drawing.Imaging.ImageFormat.Jpeg);
                                }
                                ViewBag.Title = screen.Screen_Title_Tx;
                                ViewBag.MenuId = menuid;
                                ViewBag.ActionClass = act;
