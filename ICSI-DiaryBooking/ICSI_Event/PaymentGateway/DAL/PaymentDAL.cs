...
namespace PaymentGateway.DAL
{
    class PaymentDAL
    {
        ...
        public int UpdatePaymentDal(string connectionStringPg2,string TxnID, string TxnDate, string TxnCode, string TxnResponse, int RequestID, int paymentstatus,string paymode,double ST,double ProcFee,double TotAmt,double convfee)
        {
            int UpdID = 0;
            try
            {
        ...
        public string GetRegIdDAL(string connectionstringpg2,string regid)
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    string query = "SELECT PROCESS_REQUEST_ID FROM ONLINE_PAYMENT_DETAIL_T WHERE ID="+regid+"";
